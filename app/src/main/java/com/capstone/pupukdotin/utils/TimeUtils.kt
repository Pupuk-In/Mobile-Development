...
fun String.convertTime(newPattern: String, oldPattern: String? = null): String {
    val output = SimpleDateFormat(newPattern, Locale("id", "ID"))
    var outputString = this
    try {
        val input = SimpleDateFormat(oldPattern ?:"yyyy-MM-dd'T'HH:mm:ss.SSS'Z'", Locale("id", "ID")).parse(this)
